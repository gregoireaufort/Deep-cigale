...
def fit_good_SNR(SNR):
        
    good_files = []
    for folder_path in glob.glob("test_moons/ID"+"*") :
        list_files = glob.glob(folder_path+"/*") 
        for file in list_files:
            hdul = fits.open(file)
            noise_array =  hdul[8].data
            
            no_noise = hdul[4].data
            mSNR = np.mean(no_noise/noise_array)
            if mSNR > SNR:
...
for file in good_files[1:]:
    CIGALE_parameters["file_store"] = 'test_moons/res/'+file[33:len(file)-4] + "csv"
    file_res.append(analyse_results(CIGALE_parameters))