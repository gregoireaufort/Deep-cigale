...
class ParametersManagerGrid:
    ...
    def __init__(self, conf):
        ...
        deep_modules = [name for name in self.modules if "deep" in name]
        
        for module in deep_modules:
            deep_parameters = self._param_dict_combine(conf['sed_modules_params'][module])
            new_keys = [[module +"."+ list(deep_parameters[j].keys())[i]
                        for i in
                        range(len(deep_parameters[j]))]
                        for j in range(len(deep_parameters))]
            new_parameters =[dict(zip(new_keys[i],list(deep_parameters[i].values())))
                             for i in range(len(new_keys))]
            path_data = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))+'/data/'
            path  = path_data + str(module) + "_parameters.csv"
            pd.DataFrame.from_dict(new_parameters).to_csv( path, index = False,sep =" ")
