...
def Deep_cloudy(params_cloudy, path_cloudy):
    ...
    scaler = joblib.load(path_cloudy+"X_scaling.pkl")
    X= scaler.transform(X)
    
    pca = joblib.load(path_cloudy+"pca_trained.pkl")
    ANN = joblib.load(path_cloudy+"ANN/13_04_27_06.pkl")
    y_pred = ANN.predict(X)
    ANN_hbeta = joblib.load(path_cloudy+"ANN/ANN_hbeta_256.pkl")
    Hb = np.exp(ANN_hbeta.predict(X))
    norm_lines = pca.inverse_transform(y_pred)
    ANN_logQ = joblib.load(path_cloudy+"ANN/ANN_logQ.pkl")
    logQ = 10**ANN_logQ.predict(X)
    line_names = []
    with open(path_cloudy+'list_lines.txt', 'r') as filehandle:
    ...
    lines_df2=lines_df2.divide(logQ, axis = 0)
