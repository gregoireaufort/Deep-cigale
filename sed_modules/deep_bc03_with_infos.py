...
# Author: Yannick Roehlly & Gregoire Aufort
...
def deep_approx_BC03():
    #try:
        ...
        params_bc03 = read_csv(path_data+'deep_bc03_with_infos_parameters.csv',sep=" ")
        params = pd.concat([params_sfh,params_bc03],axis = 1)
        ...
        infos_table = load_infos(params['deep_bc03_with_infos.metallicity'])
        infos = cut_infos(infos_table,
                          params['deep_bc03_with_infos.metallicity'],
                          params["deep_sfhdelayed.age_main"])
        
        labelencoder = LabelEncoder()
        labelencoder.classes_ = np.load(path_data +'classes_metallicity.npy')
        met_enc = labelencoder.transform(params['deep_bc03_with_infos.metallicity'])
        mat_params = concat([test_nn,params['deep_bc03_with_infos.metallicity']], axis = 1).values
        ...
        mat_params[:,5] = params['deep_bc03_with_infos.metallicity']
        datadb = dict()
        for i in range(n):
