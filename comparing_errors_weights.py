...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  6 14:37:12 2022
...
error2 ={}
error2["photo"] = pd.DataFrame()
error2["spectro"] = pd.DataFrame()
error2["photo+spectro"] = pd.DataFrame()
for ident in range(20):
    ...
    for mode in [["photo"],["spectro"],["spectro","photo"]]:
        ...
        file_store2 = 'test_Jorge/complete2/'+str(ident)+"_"+title+ ".csv"
        CIGALE_parameters["file_store"] = file_store2
        res = SED_statistical_analysis.analyse_results(CIGALE_parameters)
        temp = {}
        for parameter in res.keys():
                   temp[parameter]= [np.abs((res[parameter]["mean"] - fit_jorge[parameter]) /fit_jorge[parameter])]
        error2[title] = pd.concat([error2[title],pd.DataFrame.from_dict(temp)])


error["spectro"].describe() 
error2["spectro"].describe()
...
error2["photo+spectro"]["mode"] = "photo+spectro"
error2["photo"]["mode"] = "photo"
error2["spectro"]["mode"] = "spectro"



error["photo+spectro"]["weigthing"] = "Wavelength"
error["photo"]["weigthing"] = "Wavelength"
error["spectro"]["weigthing"] = "Wavelength"


error2["photo+spectro"]["weigthing"] = "One"
error2["photo"]["weigthing"] = "One"
error2["spectro"]["weigthing"] = "One"

cdf = pd.concat([error2["photo"],error2["spectro"],error2["photo+spectro"],error["photo"],error["spectro"],error["photo+spectro"] ])
mdf = pd.melt(cdf, id_vars = ["weigthing"], value_vars = [*res.keys()])


import seaborn as sns
sns.boxplot(x = "variable", y ="value",hue = "weigthing",data = mdf)
plt.yscale("log")
